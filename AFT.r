...
gen.data.lod <- function (n, mr, lod) {
  X1 <- rweibull(n, 1, 1/3)   
  X2 <- rnorm(n, 0, 0.8)
  Z  <- rnorm(n, 0, 1)
...
One.simulation.AFT.lod <- function(n, mr, lod ) { 
  # Generating data:
  data <- gen.data.lod(n, mr, lod )
  # M1-  CC: Complete case
...
set.seed(012)
One.simulation.AFT.lod(100,.2,0)
