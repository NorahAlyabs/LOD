#################################################################### 
#                                                                  #
#     Simulation for AFT model with correlated covariates          #
#                                                                  #
####################################################################
...
#'Define variables:
#'
#' param n is the sample size n = 50, 100
...
#' @param Ti Failure time, log(Ti) = -2 + X1 - X2 +  Z + e
...
#' @param rho1 the correlation level, we used 0.4


# 1. Generating data:

gen.data.lod.correlated <- function (n, mr, rho1, lod) { 
  copula1 <- rCopula (n, claytonCopula (iRho (claytonCopula ( ), rho1)))
  X1 <- qweibull(copula1[,1], 1, 1/3)
  X2 <- rnorm(n, 0, 0.8)
  Z  <- qnorm(copula1[,2], 0, 1)
...
One.simulation.AFT.lod.correlated <- function(n, mr, rho1, lod) { 
  # Generating data:
  data <- gen.data.lod.correlated(n, mr, rho1, lod)
  # M1-  CC: Complete case
  if (with(data, sum(Delta * V1 * V2)) > 2)
  {CCA <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z, data = data, subset = V1 * V2 > 0 ,
  control = survreg.control(maxiter=31), dist = "lognormal")
  if (CCA$iter == 31 ) { CC1 <- rep(NA,6)
  } else { if (dim(vcov(CCA))[1] == 5) 
  {CC1 <- c(coef(CCA)[2:4], diag(vcov(CCA))[2:4])
  } else {CC1 <- c(coef(CCA)[2:4], rep(NA, 3))}}
  } else {CC1 <- rep(NA, 6)}
  ...
  sub1 <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z , data = dat1 , dist = "lognormal",  control = survreg.control(maxiter=31))
  if (sub1$iter == 31 ) { Sub1 <- rep(NA,6)
  } else {  if (dim(vcov(sub1))[1] == 5) 
  ...
  sub2 <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z , data = dat2 ,
  dist = "lognormal",control = survreg.control(maxiter=31))
  if (sub2$iter == 31 ) { Sub2 <- rep(NA,6)
  } else { if (dim(vcov(sub2))[1] == 5) 
  ...
  L1 <- min(data$X1) ;  L2 <- min(data$X2)
  U1 <- max(data$X1) ;  U2 <- max(data$X2)
  ...
  truncNor <- function(para, X, V, l, u) {
    ...
    s1 <- para[2]
    # Likelihood function
    - sum(dnorm(Xo, mu, s1, log = T)) - r1 * log(pnorm(l, mu, s1)) - r2 *log(1- pnorm(u, mu, s1))^min(r2,1) 
    }
  ...
  MLE1 <- optim(par = para[1:2], fn = truncNor, X = data$X1, 
                V = data$V1, l = L1, u = U1)[["par"]]
  MLE2 <- optim(par = para[3:4], fn = truncNor, X = data$X2, 
                V = data$V2, l = L2, u = U2)[["par"]]
  ...
  dat4$X1[data$V1 == 0 & data$X1 == L1 ] <- MLE1[1] - MLE1[2] *  exp(phi(L1, MLE1))/ Phi(L1, MLE1)
  dat4$X2[data$V2 == 0 & data$X2 == L2 ] <- MLE2[1] - MLE2[2] *  exp(phi(L2, MLE2))/ Phi(L2, MLE2)
  dat4$X1[data$V1 == 0 & data$X1 == U1 ] <- MLE1[1] + MLE1[2] *  exp(phi(U1, MLE1))/ (1 - Phi(U1, MLE1))
  dat4$X2[data$V2 == 0 & data$X2 == U2 ] <- MLE2[1] + MLE2[2] *  exp(phi(U2, MLE2))/(1 - Phi(U2, MLE2))
  
  #Fitting AFT model and remove divergence cases if any.
  Si <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z , data = dat4 , dist = "lognormal",control = survreg.control(maxiter=31))
  if (Si$iter == 31 ) { SI <- rep(NA,6)
  } else { if (dim(vcov(Si))[1] == 5)
  ...
  dist ="lognormal",control = survreg.control(maxiter=31))))
  if (all(PMM$glanced$iter) == 31 ) { PMM1 <- rep(NA,6)
  } else { if (sum(is.na(summary(PMM))) > 0) {PMM1 <- rep(NA,6)
  ...
  Sub4 <- function(i, dat = dat5, V){
    ...
    if (V == 0){ #M6 without extending
      sub4 <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z, data = dat , 
      dist = "lognormal",control = survreg.control(maxiter=31))
    }
    if (V == 2){ # Extending with MID
      sub4 <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z + I(1 - V1) + I(1 - V2) + 
      I(Z*X1) +I(Z*X2), data = dat , 
      dist = "lognormal",control = survreg.control(maxiter=31))
    }
    if (V == 3){ #Extending with EMDI
      sub4 <- survreg(Surv(Y, Delta) ~ X1 + X2 + Z + V1 + V2 + I(X1 * (1- V1)) +
      I(X2 * (1- V2)) + I(X2 * Z)  + I(X1*Z),  data = dat , dist = "lognormal",
      control = survreg.control(maxiter=31))
    }
    if (sub4$iter == 31 ) { Sub4 <- rep(NA,6)
    } else { if (dim(vcov(sub4))[1] == 5 + 2 * V  )
  ...
  MDI <- survreg(Surv(Y, Delta) ~ I(X1 * V1) + I(X2 * V2) + Z + I(1 - V1) + 
  I(1 - V2), data = data , dist = "lognormal",control = survreg.control(maxiter=31))
  if (MDI$iter == 31 ) { MDI1 <- rep(NA,6)
  } else { if (dim(vcov(MDI))[1] == 7) 
  {MDI1 <- c(coef(MDI)[2:4], diag(vcov(MDI))[2:4])
  } else {MDI1 <- c(coef(MDI)[2:4], rep(NA, 3))}}
  
  # M8- Extended missing indicator approach (EMDI):
  EMDI <- survreg(Surv(Y, Delta) ~ I(X1 * V1) + I(X2 * V2) + Z + I(1 - V1) + 
  I(1 - V2) + I(Z * (1 - V1)) + I(Z * (1 - V2)), data = data, 
  dist = "lognormal",control = survreg.control(maxiter=31))
  if (EMDI$iter == 31 ) { EMDI1 <- rep(NA,6)
  } else { if (dim(vcov(EMDI))[1] == 9) 
  {EMDI1 <- c(coef(EMDI)[2:4], diag(vcov(EMDI))[2:4])
  } else {EMDI1 <- c(coef(EMDI)[2:4], rep(NA, 3))}}
  
  # M9- combine MDI(m7) and MI(M5):
  MIM <- mice::pool(with(imp, survreg(Surv(Y, Delta) ~ X1 + X2 + Z + I(1 - V1) + 
  I(1 - V2) + I(Z*X1) +I(Z*X2) ), dist ="lognormal",control = survreg.control(maxiter=31)))
  if (all(MIM$glanced$iter) == 31 ) { MIM1 <- rep(NA,6)
  } else { if (sum(is.na(summary(MIM))) > 0) {MIM1 <- rep(NA,6)
  } else { MIM1 <- c(summary(MIM)[2:4,2], summary(MIM)[2:4, 3]^2 )}}
  ...
  # M11- combine EMDI(M8) and MI(M5):
    EMIM <- mice::pool(with(imp, survreg(Surv(Y, Delta) ~ X1 + X2 + Z + V1 + V2 +
  I(X1 * (1- V1)) +I(X2 * (1- V2)) + I(X2 * Z)  + I(X1*Z) ), dist ="lognormal",
  control = survreg.control(maxiter=31)))
  if (all(EMIM$glanced$iter) == 31 ) { EMIM1 <- rep(NA,6)
  } else { if (sum(is.na(summary(EMIM))) > 0) {EMIM1 <- rep(NA,6)
  } else { EMIM1 <- c(summary(EMIM)[2:4,2], summary(EMIM)[2:4, 3]^2 )}}
...
set.seed(012)
One.simulation.AFT.lod.correlated(100, 0.2, 0.4, 0)
